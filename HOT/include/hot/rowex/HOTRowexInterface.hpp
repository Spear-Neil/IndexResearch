...
struct HOTRowex {
 ...
 private:
  ...
  inline idx::contenthelpers::OptionalValue<bool> insertForStackRange(
    InsertStackType& insertStack, const HOTRowexFirstInsertLevel<InsertStackEntryType>& insertLevel,
    unsigned int numberLockedEntries, ValueType const& valueToInsert
  );

  static inline void leafNodePushDown(
    InsertStackEntryType& leafEntry, hot::commons::InsertInformation const& insertInformation,
    HOTRowexChildPointer const& valueToInsert
  );

  static inline void normalInsert(
    InsertStackEntryType& currentNodeStackEntry, hot::commons::InsertInformation const& insertInformation,
    HOTRowexChildPointer const& valueToInsert
  );

  static inline hot::commons::BiNode<HOTRowexChildPointer> split(
    InsertStackEntryType& currentInsertStackEntry, hot::commons::InsertInformation const& insertInformation,
    HOTRowexChildPointer const& valueToInsert
  );
 ...
 public:
  ...
  void statistics() {
    std::cout << "-- hot statistics:" << std::endl;
    std::map<std::string, double> stat;
    collectStatsForSubtree(mRoot, stat);

    std::cout << "\tindex size\t" << stat["total"] << " Byte, "
              << double(stat["total"]) / (1024ul * 1024) << " MB, " <<
              double(stat["total"]) / (1024ul * 1024 * 1024) << " GB" << std::endl;
    stat.erase("total");
    std::cout << "\theight\t" << mRoot.getHeight() << std::endl;

    std::map<size_t, size_t> leafNodesPerDepth;
    getValueDistribution(mRoot, 0, leafNodesPerDepth);

    double leaf_num = 0;
    for(auto item : leafNodesPerDepth) leaf_num += item.second;
    std::cout << "\n\tdepth\tleaf num\tratio" << std::endl;
    for(auto item : leafNodesPerDepth) {
      std::cout << "\t" << item.first << "\t" << item.second
                << "\t" << double(item.second) / leaf_num << std::endl;
    }

    double node_num = 0;
    for(auto item : stat) node_num += item.second;
    std::cout << "\n\tnode type\tnode num\tratio" << std::endl;
    for(auto item : stat) {
      std::cout << "\t" << item.first << "\t" << size_t(item.second)
                << "\t" << item.second / node_num << std::endl;
    }
  }

 private:
  inline void
  collectStatsForSubtree(HOTRowexChildPointer const& subTreeRoot, std::map<std::string, double>& stats) const;

  inline void getValueDistribution(HOTRowexChildPointer const& childPointer, size_t depth,
                                   std::map<size_t, size_t>& leafNodesPerDepth) const;

  inline void getBinaryTrieValueDistribution(HOTRowexChildPointer const& childPointer, size_t binaryTrieDepth,
                                             std::map<size_t, size_t>& leafNodesPerDepth) const;
