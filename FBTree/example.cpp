...
void simple_test(size_t nkey, int nthd, bool shuffle) {
  ...
  for(int tid = 0; tid < nthd; tid++) {
    workers.push_back(std::thread([&](int tid) {
      ...
      for(size_t i = begin; i < end; i++) {
        ...
        void* old = tree.upsert(data[i], data[i]);
        if(old != nullptr) {
  ...
  for(int tid = 0; tid < nthd; tid++) {
    workers.push_back(std::thread([&](int tid) {
      ...
      for(size_t i = begin; i < end; i++) {
        ...
        void* old = tree.update(data[i], data[i]);
        if(old == nullptr) {
  ...
  for(int tid = 0; tid < nthd; tid++) {
    workers.push_back(std::thread([&](int tid) {
      ...
      for(size_t i = begin; i < end; i++) {
        EpochGuard epoch_guard(tree.get_epoch());
        KVPair<K, K>* kv = tree.lookup(data[i]);
        if(kv == nullptr) {
  ...
  for(int tid = 0; tid < nthd; tid++) {
    workers.push_back(std::thread([&](int tid) {
      ...
      for(size_t i = begin; i < end; i++) {
        EpochGuard epoch_guard(tree.get_epoch());
        KVPair<K, K>* kv = tree.remove(data[i]);
...
int main(int argc, char* argv[]) {
  ...
  simple_test<int>(nkey, nthd, shuffle);
  return 0;
}